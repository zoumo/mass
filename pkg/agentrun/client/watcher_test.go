...
func makeNotifMsg(t *testing.T, ev runapi.AgentRunEvent) jsonrpc.NotificationMsg {
	t.Helper()
	raw, err := json.Marshal(ev)
	require.NoError(t, err)
	return jsonrpc.NotificationMsg{
		Method: runapi.MethodRuntimeEventUpdate,
		Params: raw,
	}
}
...
func TestWatcher_DeliverMatchingEvents(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		notifCh <- makeNotifMsg(t, runapi.AgentRunEvent{
			...
			Payload: runapi.NewContentEvent(runapi.EventTypeAgentMessage, "", runapi.TextBlock("hello")),
		})
	}
...
func TestWatcher_FilterByWatchID(t *testing.T) {
	...
	notifCh <- makeNotifMsg(t, runapi.AgentRunEvent{
		...
		Seq:     0,
		Type:    runapi.EventTypeAgentMessage,
		Payload: runapi.NewContentEvent(runapi.EventTypeAgentMessage, "", runapi.TextBlock("wrong")),
	})
	notifCh <- makeNotifMsg(t, runapi.AgentRunEvent{
		WatchID: "w-1",
		RunID:   "run-1",
		Seq:     1,
		Type:    runapi.EventTypeAgentMessage,
		Payload: runapi.NewContentEvent(runapi.EventTypeAgentMessage, "", runapi.TextBlock("right")),
	})
...
func TestWatcher_UnmarshalFailure_Skipped(t *testing.T) {
	...
	notifCh <- makeNotifMsg(t, runapi.AgentRunEvent{
		...
		Payload: runapi.TurnEndEvent{StopReason: "end_turn"},
	})
	...
	case ev := <-w.ResultChan():
		...
		assert.Equal(t, runapi.EventTypeTurnEnd, ev.Type)
	case <-time.After(2 * time.Second):
...
func TestWatcher_SlowConsumer_DropsEvent(t *testing.T) {
	...
	for i := 0; i < total; i++ {
		notifCh <- makeNotifMsg(t, runapi.AgentRunEvent{
			...
			Seq:     i,
			Type:    runapi.EventTypeAgentMessage,
			Payload: runapi.NewContentEvent(runapi.EventTypeAgentMessage, "", runapi.TextBlock("flood")),
		})
	}
