// Package client — Watcher implements K8s-style watch semantics for agent-run events.
//
...
func (w *Watcher) ResultChan() <-chan runapi.AgentRunEvent {
	return w.ch
}
...
func newWatcher(watchID string, nextSeq int, notifCh <-chan jsonrpc.NotificationMsg, unsub func()) *Watcher {
	...
	eventCh := make(chan runapi.AgentRunEvent, 256)
