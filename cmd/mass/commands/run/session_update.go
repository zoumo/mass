...
func buildSessionUpdate(ev runapi.Event) (changed []string, reason string, apply func(*apiruntime.State)) {
...
// ── Convert: runapi → apiruntime AvailableCommands ──────────────────────────

func convertToStateCommands(cmds []runapi.AvailableCommand) []apiruntime.AvailableCommand {
	if len(cmds) == 0 {
		return nil
	}
...
func convertToStateCommandInput(inp *runapi.AvailableCommandInput) *apiruntime.AvailableCommandInput {
	if inp == nil {
		return nil
	}
...
// ── Convert: runapi → apiruntime ConfigOptions ──────────────────────────────

func convertToStateConfigOptions(opts []runapi.ConfigOption) []apiruntime.ConfigOption {
	if len(opts) == 0 {
		return nil
	}
...
func convertToStateConfigOptionSelect(s *runapi.ConfigOptionSelect) *apiruntime.ConfigOptionSelect {
...
func convertToStateConfigSelectOptions(opts runapi.ConfigSelectOptions) apiruntime.ConfigSelectOptions {
	switch {
...
func convertToStateConfigSelectOptionSlice(opts []runapi.ConfigSelectOption) []apiruntime.ConfigSelectOption {
...
// ── Convert: runapi → apiruntime SessionInfo ────────────────────────────────

func convertToStateSessionInfo(e runapi.SessionInfoEvent) *apiruntime.SessionInfo {
...
// ── Convert: runapi → apiruntime CurrentMode ────────────────────────────────

func convertToStateCurrentMode(e runapi.CurrentModeEvent) *string {
	if e.ModeID == "" {
		return nil
	}
	s := e.ModeID
