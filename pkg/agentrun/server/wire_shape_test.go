...
func TestWireShape_ToolCallContent_Content(t *testing.T) {
	...
	runObj := runapi.ToolCallContent{Content: &runapi.ToolCallContentContent{
		Content: runapi.TextBlock("hi"),
	}}
	assert.Equal(t, jsonKeys(t, acpObj), jsonKeys(t, runObj), "ToolCallContent content variant")
}
...
func TestWireShape_ToolCallContent_Diff(t *testing.T) {
	...
	runObj := runapi.ToolCallContent{Diff: &runapi.ToolCallContentDiff{Path: "a.go", NewText: "x"}}
	assert.Equal(t, jsonKeys(t, acpObj), jsonKeys(t, runObj), "ToolCallContent diff variant")
}
...
func TestWireShape_ToolCallContent_Terminal(t *testing.T) {
	...
	runObj := runapi.ToolCallContent{Terminal: &runapi.ToolCallContentTerminal{TerminalID: "t1"}}
	assert.Equal(t, jsonKeys(t, acpObj), jsonKeys(t, runObj), "ToolCallContent terminal variant")
}
...
func TestWireShape_AvailableCommandInput_Unstructured(t *testing.T) {
	...
	runObj := runapi.AvailableCommandInput{
		Unstructured: &runapi.UnstructuredCommandInput{Hint: "flags here"},
	}
	acpKeys := jsonKeys(t, acpObj)
	oarKeys := jsonKeys(t, runObj)
...
func TestWireShape_ConfigOption_Select_Ungrouped(t *testing.T) {
	...
	runObj := runapi.ConfigOption{Select: &runapi.ConfigOptionSelect{
		ID: "model", Name: "Model", CurrentValue: "fast",
		Options: runapi.ConfigSelectOptions{Ungrouped: []runapi.ConfigSelectOption{{Name: "fast", Value: "fast"}}},
	}}
	...
	oarOptJSON, _ := json.Marshal(runObj)
...
func TestWireShape_ConfigOption_Select_Grouped(t *testing.T) {
	...
	runObj := runapi.ConfigOption{Select: &runapi.ConfigOptionSelect{
		ID: "model", Name: "Model", CurrentValue: "opt",
		Options: runapi.ConfigSelectOptions{Grouped: []runapi.ConfigSelectGroup{
			{Group: "g1", Name: "Group 1", Options: []runapi.ConfigSelectOption{{Name: "opt", Value: "opt"}}},
		}},
	}}
...
func TestWireShape_UnionMultiVariant_MarshalErrors(t *testing.T) {
	t.Run("ToolCallContent multi-variant", func(t *testing.T) {
		tc := runapi.ToolCallContent{
			Diff:     &runapi.ToolCallContentDiff{Path: "a.go", NewText: "x"},
			Terminal: &runapi.ToolCallContentTerminal{TerminalID: "t1"},
		}
	...
	t.Run("ConfigSelectOptions multi-variant", func(t *testing.T) {
		cso := runapi.ConfigSelectOptions{
			Ungrouped: []runapi.ConfigSelectOption{{Name: "a", Value: "a"}},
			Grouped:   []runapi.ConfigSelectGroup{{Group: "g", Name: "G", Options: nil}},
		}
...
func TestWireShape_ConfigSelectOptions_EmptyMarshalError(t *testing.T) {
	_, err := json.Marshal(runapi.ConfigSelectOptions{})
	require.Error(t, err)
	assert.Contains(t, err.Error(), "ConfigSelectOptions")
}
