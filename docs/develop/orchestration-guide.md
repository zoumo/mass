...
# 轮询等待 ready
while true; do
  phase=$(massctl workspace get --name my-project -o json | jq -r '.status.phase')
  [ "$phase" = "ready" ] && break
  sleep 1
done
```
...
# 轮询等待 idle
while true; do
  state=$(massctl agentrun get --workspace my-project --name coder -o json | jq -r '.status.state')
  [ "$state" = "idle" ] && break
...
```bash
# 发送 prompt（异步，立即返回）
massctl agentrun prompt --workspace my-project --name coder --text "Implement function X"
...
```bash
massctl agentrun stop --workspace my-project --name coder
massctl agentrun stop --workspace my-project --name reviewer
massctl agentrun delete --workspace my-project --name coder
massctl agentrun delete --workspace my-project --name reviewer
massctl workspace delete --name my-project
```
...
wait_for_idle() {
  local ws=$1 name=$2
  while true; do
    state=$(massctl agentrun get --workspace "$ws" --name "$name" -o json | jq -r '.status.state')
    case "$state" in
      idle) return 0 ;;
      error) return 1 ;;
      stopped) return 2 ;;
...
| `massctl agentrun prompt --workspace <ws> --name <name> --text <msg>` | 发送 prompt |
| `massctl workspace send --name <ws> --from <a> --to <b> --text <msg>` | agent 间通信 |
| `massctl agentrun stop --workspace <ws> --name <name>` | 停止 agent |
| `massctl agentrun delete --workspace <ws> --name <name>` | 删除 agent |

### 工作流程

1. 确保 mass daemon 已启动（`massctl daemon status`）
2. 注册需要的 agent 定义（或确认已存在：`massctl agent list`）
...
```bash
target_state=$(massctl agentrun get --workspace ws --name reviewer -o json | jq -r '.status.state')
...
```bash
massctl agentrun list --workspace ws -o json | jq '.items[] | {name: .metadata.name, state: .status.state}'
```
