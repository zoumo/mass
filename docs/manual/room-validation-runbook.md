...
| **claude-code** | `claude-code` | 复查 review 结果，标注优先级 | `@agentclientprotocol/claude-agent-acp` (Node) |
| **gsd-pi** | `gsd-pi` | 按复查结果执行修复 | `pi-acp` (Node) |
...
配置文件（`bin/e2e/config.yaml`）：

```yaml
socket: /Users/jim/code/zoumo/open-agent-runtime/bin/e2e/agentd.sock
workspaceRoot: /Users/jim/code/zoumo/open-agent-runtime/bin/e2e/workspaces
metaDB: /Users/jim/code/zoumo/open-agent-runtime/bin/e2e/agentd.db
bundleRoot: /Users/jim/code/zoumo/open-agent-runtime/bin/e2e/bundles
...
预期输出：
```
agentd: loaded config from bin/e2e/config.yaml
...
> Agent 的身份标识为 `oar-project/codex`、`oar-project/claude-code`、`oar-project/gsd-pi`，即 `workspace/name` 格式。
...
```bash
$CTL agent status oar-project/codex
$CTL agent status oar-project/claude-code
$CTL agent status oar-project/gsd-pi
...
{
  "agents": [
    {"workspace": "oar-project", "name": "codex", "runtimeClass": "codex", "state": "idle"},
    {"workspace": "oar-project", "name": "claude-code", "runtimeClass": "claude-code", "state": "idle"},
    {"workspace": "oar-project", "name": "gsd-pi", "runtimeClass": "gsd-pi", "state": "idle"}
  ]
}
```
...
```bash
$CTL agent prompt oar-project/codex --wait --text '
...
完成后，调用 workspace_send 工具把 review 结果发给 claude-code。
'
```
...
> **workspace/send 工作原理：** 消息会被添加归因前缀 `[workspace:oar-project from:codex]`，然后通过 `agent/prompt` 投递到 claude-code agent 的关联 shim。这是异步的 — 命令在投递后返回 `{delivered: true/false}`。

#### 6.3 让 Claude Code 复查

```bash
$CTL agent prompt oar-project/claude-code --wait --text '
你收到了 codex agent 的 code review 结果。请：
1. 验证每个发现是否属实（检查实际代码）
...
完成后，调用 workspace_send 把最终修复清单发给 gsd-pi。
'
```
...
```bash
$CTL agent prompt oar-project/gsd-pi --wait --text '
...
```bash
# 停止所有 agent
$CTL agent stop oar-project/codex
$CTL agent stop oar-project/claude-code
$CTL agent stop oar-project/gsd-pi

# 删除已停止的 agent（必须先 stop）
$CTL agent delete oar-project/codex
$CTL agent delete oar-project/claude-code
$CTL agent delete oar-project/gsd-pi
...
# 正确
$CTL agent status oar-project/codex
$CTL agent prompt oar-project/claude-code --text "..."
$CTL agent stop oar-project/gsd-pi
...
# agent/create
echo '{"jsonrpc":"2.0","id":3,"method":"agent/create","params":{"workspace":"oar-project","name":"codex","runtimeClass":"codex"}}' \
  | socat - UNIX-CONNECT:$SOCK

# agent/prompt
echo '{"jsonrpc":"2.0","id":4,"method":"agent/prompt","params":{"workspace":"oar-project","name":"codex","prompt":"review the code"}}' \
  | socat - UNIX-CONNECT:$SOCK

# workspace/send
echo '{"jsonrpc":"2.0","id":5,"method":"workspace/send","params":{"workspace":"oar-project","from":"codex","to":"claude-code","message":"review results..."}}' \
  | socat - UNIX-CONNECT:$SOCK

# agent/status
echo '{"jsonrpc":"2.0","id":6,"method":"agent/status","params":{"workspace":"oar-project","name":"codex"}}' \
  | socat - UNIX-CONNECT:$SOCK
```
...
|---|---|---|
| **claude-code** | `claude-code` | Code review + 复查 |
| **gsd-pi** | `gsd-pi` | 执行修复 |

流程：
1. Claude 做 review → `workspace send` 给 gsd-pi
2. GSD 执行修复
...
- **消息归因是文本前缀** — `[workspace:oar-project from:codex]` 是 prepend 在 prompt 前面的，不是结构化元数据。
