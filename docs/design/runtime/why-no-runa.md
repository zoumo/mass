...
| runc 的职责 | MASS 中的归属 |
|-------------|-------------|
| fork/exec 进程 | agent-run（启动 agent 进程，同时持有 stdio） |
| 持有 stdio | agent-run（唯一的 ACP client） |
| 内核隔离 | 不需要 |
| 写 state.json | agent-run（维护 session 状态） |
| 生命周期操作（create/kill/delete） | agent-run RPC（`session/prompt`、`session/cancel`、`runtime/stop`、`runtime/status`） |

MASS 的结论是：**在 agent 世界里，"启动进程"和"持有 stdio 作为 ACP client"
必须由同一个常驻进程完成。这个进程是 agent-run，没有 runa 的位置。**
...
```
mass → agent-run（fork/exec + 持有 stdio + ACP client）→ agent 进程
              ↑
         必须常驻，因为它是 ACP client
```

runc 的存在是因为 containerd-shim 和 runc 可以分工。
agent-run 把这两者合并了，因为在 ACP 协议约束下分工没有意义。

## MASS Runtime Spec 的定位

`config.json`（MASS Runtime Spec）不消失，它的定位是：

**agentd 生成、agent-run 消费的 session 配置描述。**

它解决了 agentd 和 agent-run 之间的接口问题——agentd 不需要知道 agent-run 的命令行
参数细节，只需要把所有信息写进 config.json，agent-run 读取并执行。

这和 OCI config.json 的作用完全一致，只是消费方是 agent-run 而不是 runc。
