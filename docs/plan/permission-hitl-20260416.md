...
**文件**: `pkg/shim/api/event_constants.go`
...
**文件**: `pkg/shim/api/event_types.go`
...
**文件**: `pkg/shim/api/shim_event.go` — `decodeEventPayload` 增加两个 case
...
**文件**: `pkg/shim/runtime/acp/runtime.go`
...
**文件**: `pkg/shim/runtime/acp/client.go`
...
**文件**: `pkg/shim/server/service.go`

```go
func (s *Service) ResolvePermission(ctx context.Context, req *apishim.SessionResolvePermissionParams) (*apishim.SessionResolvePermissionResult, error) {
    if err := s.mgr.ResolvePermission(req.RequestID, req.OptionID); err != nil {
        return nil, jsonrpc.ErrInvalidParams(err.Error())
    }
    return &apishim.SessionResolvePermissionResult{}, nil
}
```

**文件**: `pkg/shim/server/register.go` — 注册 `session/resolve_permission` 方法

---

## 7. Client 层同步更新

**文件**: `pkg/shim/client/client.go`
...
callback 内部调用 `Translator.broadcastSessionEvent()`。
这与现有 `StateChangeHook` 模式一致。
...
| `pkg/shim/api/event_constants.go` | +`EventTypePermissionRequest`, +`EventTypePermissionResolved` |
| `pkg/shim/api/event_types.go` | +`PermissionRequestEvent`, +`PermissionResolvedEvent`, +支持类型 |
| `pkg/shim/api/shim_event.go` | `decodeEventPayload` 增加 permission 事件 case |
| `pkg/shim/api/methods.go` | +`MethodSessionResolvePermission` |
| `pkg/shim/api/types.go` | +`SessionResolvePermissionParams/Result` |
| `pkg/shim/runtime/acp/client.go` | `RequestPermission()` 增加 `manual` 分支 |
| `pkg/shim/runtime/acp/runtime.go` | +`pendingPerm` 结构, +`ResolvePermission()`, +`PermissionHook` |
| `pkg/shim/server/service.go` | +`ResolvePermission()` handler |
| `pkg/shim/server/register.go` | 注册 `session/resolve_permission` |
| `pkg/shim/client/client.go` | +`ResolvePermission()` client 方法 |
