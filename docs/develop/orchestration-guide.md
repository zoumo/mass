...
# 轮询等待 idle
while true; do
  state=$(massctl agentrun get coder -w my-project -o json | jq -r '.status.state')
  [ "$state" = "idle" ] && break
...
wait_for_idle() {
  local ws=$1 name=$2
  while true; do
    state=$(massctl agentrun get "$name" -w "$ws" -o json | jq -r '.status.state')
    case "$state" in
      idle) return 0 ;;
      error) return 1 ;;
      stopped) return 2 ;;
...
```bash
target_state=$(massctl agentrun get reviewer -w ws -o json | jq -r '.status.state')
...
```bash
massctl agentrun get -w ws -o json | jq '.items[] | {name: .metadata.name, state: .status.state}'
```
