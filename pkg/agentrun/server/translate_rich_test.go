...
func TestTranslateRich_ToolCall_FullFields(t *testing.T) {
	...
	tr := NewTranslator("s1", in, "", slog.Default())
	ch, _, _ := tr.Subscribe()
	tr.Start()
	defer tr.Stop()
	...
	ev, ok := sessionPayload(t, drainEvent(t, ch)).(runapi.ToolCallEvent)
	require.True(t, ok)
...
func TestTranslateRich_ToolCallUpdate_FullFields(t *testing.T) {
	...
	ev, ok := sessionPayload(t, drainEvent(t, ch)).(runapi.ToolResultEvent)
	require.True(t, ok)
...
func TestTranslateRich_ContentBlock_Text(t *testing.T) {
	...
	ev, ok := sessionPayload(t, drainEvent(t, ch)).(runapi.ContentEvent)
	require.True(t, ok)
...
func TestTranslateRich_AvailableCommandsUpdate(t *testing.T) {
	...
	ru, ok := sessionPayload(t, drainEvent(t, ch)).(runapi.RuntimeUpdateEvent)
	require.True(t, ok)
...
func TestTranslateRich_AgentRunEventDecode_AllEventTypes(t *testing.T) {
	cases := []struct {
		name    string
		evType  string
		payload runapi.Event
	}{
		{"agent_message", "agent_message", runapi.NewContentEvent("agent_message", "", runapi.TextBlock("hi"))},
		{"agent_thinking", "agent_thinking", runapi.NewContentEvent("agent_thinking", "", runapi.TextBlock("thought"))},
		{"user_message", "user_message", runapi.NewContentEvent("user_message", "", runapi.TextBlock("user"))},
		{"tool_call", "tool_call", runapi.ToolCallEvent{ID: "1", Kind: "shell", Title: "t"}},
		{"tool_result", "tool_result", runapi.ToolResultEvent{ID: "1", Status: "completed"}},
		{"plan", "plan", runapi.PlanEvent{Entries: nil}},
...
func TestTranslateRich_AgentRunEvent_BackwardCompat(t *testing.T) {
	...
	var se runapi.AgentRunEvent
	require.NoError(t, json.Unmarshal([]byte(tcJSON), &se))
	ev, ok := se.Payload.(runapi.ToolCallEvent)
	require.True(t, ok)
	assert.Equal(t, "tc-1", ev.ID)
	assert.Equal(t, "shell", ev.Kind)
	assert.Equal(t, "run ls", ev.Title)
	...
	var se2 runapi.AgentRunEvent
	require.NoError(t, json.Unmarshal([]byte(txtJSON), &se2))
	txt, ok := se2.Payload.(runapi.ContentEvent)
	require.True(t, ok)
