...
key_decisions:
  ...
  - D089: Unconditional best-effort session recovery — session/load always attempted during recovery; agent-run checks ACP loadSession capability internally and auto-fallbacks; no RestartPolicy
...
key_files:
  - pkg/meta/models.go — Agent, Workspace, ObjectMeta, AgentSpec; no UUID, no Session/Room, no RestartPolicy
  ...
  - pkg/agentd/recovery.go — session/load block after Subscribe; readStateSessionID helper; creating→error cleanup pass
...
lessons_learned:
  ...
  - Session recovery ordering is critical: Subscribe must be established before session/load so any immediate stateChange from the agent-run isn't missed. The atomic window between Subscribe and Load is the vulnerability
...
**Vision check:** bbolt replaces SQLite ✅; spec.Status (idle) is single state enum ✅; Session and Room concepts eliminated ✅; Workspace is unified grouping+filesystem resource ✅; Agent identity is (workspace,name) no UUID ✅; shim is sole post-bootstrap write authority ✅; unconditional best-effort session recovery ✅; no compat layer ✅; all layers (storage/model/agentd/ARI/CLI/MCP/integration) shipped ✅.
...
**R044** (restart/recovery quality attribute) — advanced→validated by M007/S02: unconditional best-effort session recovery implemented (RestartPolicy removed); shim write authority boundary (D088) enforced with unit test proof.
