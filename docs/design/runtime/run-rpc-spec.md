...
# Agent-Run RPC Specification

## 概述

本文档定义 agent-run 对上暴露的 **agent-run RPC 目标契约**。
它是 mass 与单个 runtime session 之间的本地控制面：

- `session/*` 负责 turn 级控制与订阅；
- `runtime/*` 负责进程真相、历史回放与停止；
- ACP 保持为 agent-run 内部实现细节，不直接暴露给 mass。
...
- 本文档负责 agent-run RPC 方法名、notification 名、回放 / 重连语义；
...
In mass-managed deployments, bundle/state/socket are co-located:
...
恢复调用方（通常是 mass）在重启后应执行以下顺序：

1. 使用已持久化的 AgentRun 元数据（`ShimSocketPath`）找到 socket；
2. 连接每个 agent-run socket；
...
这是 agent-run 边界上的 work-entry path，对应上层 ARI `agentrun/prompt` 的 runtime 侧落点。
...
```json
{
  ...
  "params": {
    "prompt": [{"type": "text", "text": "Refactor the auth module to use JWT tokens."}]
  }
}
```
...
- 调用期间，agent-run 会持续产出 `runtime/event_update` notification（包含 session events 与 runtime_update events）；
...
- **两阶段无锁 replay**：agent-run 先在 Translator mutex 下注册 subscriber channel（O(1)），
...
- **慢消费者驱逐（K8s 模型）**：subscriber channel buffer = 1024。当 channel 满时，
  agent-run 关闭该 subscriber 的 channel 并移除订阅，然后 `peer.Close()` 强制断开连接
...
优雅停止 runtime，并关闭 agent-run RPC server。
...
使 mass 可以在 turn 级精确排序，而不必把完整的时间戳或索引外部化到调用方。
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `turnId` | `string` | 当前 prompt turn 的标识符，由 agent-run 在 `turn_start` 事件时分配，在 `turn_end` 事件后清除 |
...
2. **`turnId` 在 `turn_start` 事件时由 agent-run 分配**，并在该 turn 内所有 `runtime/event_update` notification 中携带相同值；
...
这是 agent-run 对外暴露的统一 notification。所有来自 ACP 的文本流、工具调用、
...
- 这是 agent-run 对外的 typed surface，不是 ACP notification 的 1:1 透传；
- 上层消费方不得依赖 ACP 原始事件名；
- 若底层 agent 协议替换为非 ACP，只要 agent-run 维持此 typed surface，上层 contract 不变。
...
Agent-Run 追踪当前 open block 的 event type。只要下一个事件的类型
≠ 当前 block 的类型，agent-run 会先 emit 一个 `status: "end"` 的空内容事件
...
- mass 是 runtime/session 语义的消费者，不是 ACP router；
- agent-run 负责把 ACP `session/update`、tool requests、错误状态翻译成上层可消费事件；
- `fs/*`、`terminal/*`、原始 ACP 握手与兼容性补丁留在 agent-run 内部，并受 permission posture 约束。
...
| `runtime/stop` | 请求 / 响应 | 否 | 停止 runtime 与 agent-run |
...
| `-32603` | Internal error | agent 进程异常、ACP 通信失败、事件日志读取失败、agent-run 内部错误 |
