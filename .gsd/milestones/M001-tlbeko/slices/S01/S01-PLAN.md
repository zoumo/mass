...
- [x] **T02: Implemented GitHandler with Prepare method for cloning git repos with ref/depth support and structured GitError for agent inspection.** — Define SourceHandler interface with Prepare(ctx context.Context, source Source, targetDir string) (workspacePath string, error). Implement GitHandler struct that shells out to git CLI via exec.CommandContext. Handle three clone modes: (1) default clone (no ref) — git clone URL targetDir with --single-branch for shallow clones, (2) ref clone (branch/tag) — git clone --branch ref --single-branch URL targetDir, (3) SHA clone — clone first then git checkout SHA in targetDir. Support depth option: add --depth N --single-branch flags. Error handling: git not found (exec.LookPath or exec error), clone failure (exit code), checkout failure (exit code). Wrap all errors with context (operation phase, URL, ref). Follow pkg/runtime/runtime.go exec pattern: CommandContext, working dir, error wrapping.
  - Estimate: 1h
