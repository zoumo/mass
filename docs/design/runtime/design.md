...
ARI client sends bootstrap request (runtimeClass: claude, systemPrompt: "...")
  → mass daemon resolves runtimeClass → finds handler config (command, args, env)
  → mass daemon merges runtimeClass config + request params
...
Phase 2 — agentRoot.path: resolve the canonical working directory
  ...
  resolved cwd: "/var/lib/mass/workspaces/ws-abc" ← runtime-derived host path
...
2. mass daemon resolves runtimeClass (from Agent definition), workspace, env, permission policy, and bundle contents.
...
|---|---|---|---|
| MASS runtime object | mass daemon + runtime | AgentRun `(workspace, name)`; runtime `phase` = `creating`/`idle`/`running`/`restarting`/`stopped`/`error` | Names the runtime instance exposed through ARI and runtime state.json. |
| mass daemon AgentRun lifecycle | mass daemon | `creating` / `idle` / `running` / `restarting` / `stopped` / `error` | AgentRun lifecycle state around the runtime process. |
...
See [mass daemon documentation](../mass/mass.md) for detailed runtimeClass configuration.
...
mass daemon looks up runtimeClass config (Agent definition):
  claude:
    clientProtocol: "acp"
    command: "npx"
...
mass daemon resolves workspace path:
...
| Field | Purpose |
|---|---|
| `workspace`, `name` | AgentRun identity (stable external key) |
| `agent` | Agent definition reference (determines process command and bootstrap config) |
