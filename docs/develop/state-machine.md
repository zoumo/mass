...
唯一例外：`idle → running` 由 agentd **先写 DB** 再投递 prompt，用于防止并发 prompt 竞争。
...
| 角色 | 行为 | 代码位置 |
|------|------|----------|
| agentd | ARI `Create` 写入 DB `state=creating` | `pkg/ari/server/server.go:373` |
...
| agentd | 收到 `state_change` 通知 → `buildNotifHandler` 写 DB | `pkg/agentd/process.go:175` |
| agentd（补偿） | 若通知在 Subscribe 之前已发出而丢失，`client.Status()` 主动查询 shim 状态并同步 DB | `pkg/agentd/process.go:322-337` |
...
| 角色 | 行为 | 代码位置 |
|------|------|----------|
| agentd | **先** 原子 CAS 写 DB `idle→running` | `pkg/ari/server/server.go:445` |
| agentd | **然后** 异步投递 `session/prompt` RPC 到 shim | `pkg/ari/server/server.go:474` |
...
| agentd | `buildNotifHandler` 收到通知，写 DB `idle` | `pkg/agentd/process.go:175-184` |
...
|------|------|----------|
| shim | 进程退出 | — |
| agentd | `watchProcess` 检测到 `Done` 信号，写 DB `stopped` | `pkg/agentd/process.go:694` |
...
| 角色 | 行为 | 代码位置 |
|------|------|----------|
| agentd | 发 `runtime/stop` RPC → shim 退出 → `watchProcess` 写 DB `stopped` | `pkg/agentd/process.go:710-764` |
| agentd | 若 10 秒未退出则 SIGKILL | `pkg/agentd/process.go:750-756` |

### 7. 启动失败（→ error）

| 角色 | 行为 | 代码位置 |
|------|------|----------|
| agentd | Start goroutine 写 DB `error` + ErrorMessage | `pkg/ari/server/server.go:394-397` |

### 8. Restart（任意状态 → creating）

| 角色 | 行为 | 代码位置 |
|------|------|----------|
| agentd | 写 DB `creating` → 若有活 shim 则 Stop → 再次写 `creating` → Start 新 shim | `pkg/ari/server/server.go:556-613` |

### 9. Recovery（daemon 重启后）

| 角色 | 行为 | 代码位置 |
|------|------|----------|
| agentd | 读 shim `runtime/status` 获取真实状态，覆盖 DB | `pkg/agentd/recovery.go:128-136` |
| agentd | shim 不可达 → 标记 `stopped` | `pkg/agentd/recovery.go:91-98` |
| agentd | 卡在 `creating` → 标记 `error` | `pkg/agentd/recovery.go:156` |
...
```
agentd ARI server                      shim                              agent 进程
       │                                │                                   │
