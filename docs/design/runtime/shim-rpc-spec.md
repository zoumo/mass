...
当前仓库里的 `pkg/rpc`、`pkg/agentd/shim_client.go`、`cmd/agent-shim-cli` 仍实现
legacy `Prompt` / `Cancel` / `Subscribe` / `GetState` / `GetHistory` / `Shutdown`
和 `$/event` surface。

那是**当前实现状态**，不是本文档的规范 authority。
S02 将把实现对齐到本文档定义的 clean-break target。
任何仍引用旧名字的规划或实现说明，都应被视为 implementation lag，而非当前 API 契约。
...
1. 扫描 `/run/agentd/shim/*/agent-shim.sock` 或使用已持久化的 runtime 元数据找到 socket；
2. 连接每个 shim socket；
...
这是 shim 边界上的 work-entry path，对应上层 ARI `session/prompt` 的 runtime 侧落点。
...
- 调用期间，shim 会持续产出 `session/update` 与必要的 `runtime/stateChange` notification；
...
1. **`seq` 继续作为全局唯一序列号**，用于跨 turn 排序、断线补齐、去重。  
   任何 shim notification（包括 `runtime/stateChange`）都携带 `seq`。
...
4. **`runtime/stateChange` 不参与 turn 排序**：该 notification 只携带全局 `seq`，不携带 `turnId` / `streamSeq` / `phase`。
...
`runtime/stateChange`（不携带 turn 字段）：
...
用于暴露 runtime process truth 的状态变化，例如 `created → running`、
`running → created`、`created/running → stopped`。
