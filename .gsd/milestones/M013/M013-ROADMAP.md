...
| S02 | ARI package restructure | medium | S01 | ⬜ | After this: make build + go test ./... pass; pkg/ari/api/ has types.go+domain.go+methods.go; pkg/ari root has only api/, server/, client/ subdirs; no imports of api/ari remain. |
| S03 | Shim package restructure + api/ deletion | high | S01, S02 | ⬜ | After this: make build + go test ./... pass; pkg/shim/api/ has all shim type files; no imports of api/shim, api (methods/events), remain; api/ directory is gone. |
